...
int minKey1(int key[], bool mstSet[])
{
    ...
    for (int g = 0; g < G; g++)
        if (mstSet[g] == false && key[g] < min)
            min = key[g], min_index = g;

    return min_index;
}

int printMST1(int adj[], int n, int matriz[G][G])
{
    printf("Algoritmo Prim para instancia gr17.tsp:\n\n");
    printf("Aresta \t\tDistancia\n\n");
    for (int i = 1; i < G; i++)
        printf("%02d - %02d \t%d \n\n", adj[i], i, matriz[i][adj[i]]);
}

void primMST1(int matriz[G][G])
{
    ...
    for (int count = 0; count < G-1; count++)
    {
        ...
        if (matriz[u][g] && mstSet[g] == false && matriz[u][g] < key[g])
            adj[g] = u, key[g] = matriz[u][g];
    }

    printMST1(adj, G, matriz);
}

int minKey2(int key[], bool mstSet[])
{
    int min = INT_MAX, min_index;

    for (int f = 0; f < F; f++)
        if (mstSet[f] == false && key[f] < min)
            min = key[f], min_index = f;

    return min_index;
}

int printMST2(int adj[], int n, int matriz[F][F])
{
    printf("Algoritmo Prim para instancia fri26.tsp:\n\n");
...
void primMST2(int matriz[F][F])
{
    ...
    for (int count = 0; count < F-1; count++)
    {
        ...
        if (matriz[u][f] && mstSet[f] == false && matriz[u][f] < key[f])
            adj[f] = u, key[f] = matriz[u][f];
    }

    printMST2(adj, F, matriz);
}
...
{
    int menu;
    printf("Escolha uma instancia\n\n");
    printf("1 - gr17.tsp\n");
    printf("2 - fri26.tsp\n");
    printf("3 - brazil58.tsp\n\n");
    scanf("%d", &menu);

    if (menu == 1)
    {
        int gr17[G][G] =   {{  0, 633, 257,  91, 412, 150,  80, 134, 259, 505, 353, 324,  70, 211, 268, 246, 121},
    ...
    {
        int fri26[F][F] =  {{  0,  83,  93, 129, 133, 139, 151, 169, 135, 114, 110,  98,  99,  95,  81, 152, 159, 181, 172, 185, 147, 157, 185, 220, 127, 181},
